              ...
              <div class="modal-dialog modal-lg">
                <div class="modal-content">
                  ...
                  <div class="modal-body text-start">
                      ...
                      <div class="mb-3">
                        <label for="" class="form-label">href</label>
                        <input class="form-control" name="" id=""  placeholder="Text" v-model="content.href">
                      </div>
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal" @click="mainHTML(tag.tag, tag.endTag)">Understood</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  methods: {
    mainHTML (tag, endTag) {
      try {
        ...
        this.socket.emit('send-html', this.$route.params.id, this.formControl(tag, endTag))
        this.content.id = ''
        this.content.class = ''
        this.content.text = ''
      } catch (error) {
    ...
    secondaryHTML (tag, endTag) {
      try {
        ...
        alert('Sorry our service is crush')
      }
    },
    formControl (tag, endTag) {
      const formData = {
        startTag: `${tag}`,
        attributes: {
          id: this.content.id,
